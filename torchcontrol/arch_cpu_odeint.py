...
class HDNN(nn.Module):
    ...
    def __init__(self, layers, predictor_type, hparams, beta , CNN_dense = None, softmax=True):
        ...
        elif self.predictor_type == 'CNN': self.predictor = CNN(self.layers,CNN_dense).to(device)
    ...
    def assignNewState(self,xi):
        self.flat_w = torch.Tensor(xi[:len(self.flat_w)]).to(device)
        self.flat_wdot = torch.Tensor(xi[len(self.flat_w):2*len(self.flat_w)]).to(device)
    ...
    def fit(self, trainloader, epoch = 3, time_delta = 1, method = 'odeint', iter_accuracy = 10, ode_t = 0.25, ode_step = 10):
        ...
        for e in range(epoch): 
            for i, data in enumerate(trainloader):
                ...
                xi = scipy.integrate.odeint(func,self.xi.detach().cpu().numpy(),t,tfirst=True)
                return(xi)
                self.xi = torch.Tensor(xi[-1]).to(device)
                self.assignNewState(torch.Tensor(xi[-1]))
                del xi
                self.loadStateDict()
                self.count += 1                
