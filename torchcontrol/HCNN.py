# -*- coding: utf-8 -*-
"""
Created on Sat Feb  2 11:26:24 2019
...
class HCNN(nn.Module):
    '''
    High level wrapper of a Hamiltonian Differential Neural Network with CNN predictor
    
    :conv_layers: list of channel dimensions for conv layers e.g [1,3,12] for 1 channel ---> 12 channel
    :dense_layer: list of dimensions of linear layers of the CNN \
    ...
    '''
    def __init__(self, conv_layers, dense_layers, hparams, beta):
        ...
        if self.predictor_type == 'MLP': self.predictor = MLP(self.layers).to(device)
    ...
    def loadStateDict(self):    
        ...
        del self.predictor
        self.predictor = CNN(self.layers,CNN_dense).to(device)
    ...
    def forward(self,t,xi):
        ...
        #dxdt = torch.sparse.mm(self.F,grad_flat.t())
